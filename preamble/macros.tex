...
% Заголовок, що не додається до змісту (напр. календарний план)
\newcommand{\frontsection}[1]{%
    \clearpage\phantomsection
    \section*{#1}
}

% Два рядки підписів: роль — лінія для підпису — прізвище
\newcommand{\signatureline}[4]{%
    \par\noindent
    \begin{tabular}{@{}ll@{}}
        #1 & \makebox[4cm]{\hrulefill}~#2 \\[2.5em]
        #3 & \makebox[4cm]{\hrulefill}~#4 \\
    \end{tabular}%
}
...
% #2 - Університет
% #3 - Кафедра (шапка)
% #4 - Тема кваліфікаційної роботи
% #5 - Група
% #6 - Студент (ПІБ)
% #7 - Керівник практики від кафедри
% #8 - База практики
% #9 - Науковий керівник
\newcommand{\makepracticetitlepage}[9]{%
    \begin{titlepage}
    \thispagestyle{empty}
    \begin{center}
    \MakeUppercase{\textbf{#1}}\\[0.5em] \textbf{#2}\\[1em] \textbf{#3}\\[1.5em]
    \MakeUppercase{\textbf{Звіт}}\\[0em] про проходження передкваліфікаційної практики\\[1em] База практики: #8\\[1em] за темою кваліфікаційної роботи:\\[0em] «#4»
    \end{center}
    \vfill\noindent\hfill
    \begin{tabular}{l} Виконав студент ННІКІТІ: \\ групи: #5 \\ #6 \\[1em] Керівник практики від кафедри: \\ \parbox[t]{8cm}{\raggedright #7\strut} \\[1em] Науковий керівник: \\ \parbox[t]{8cm}{\raggedright #9\strut} \end{tabular}
    \vspace{1.5em}\begin{center} Рівне – 2026 \end{center}
    \end{titlepage}
}
